...
LEARN_TRIGGERS = ["apprends", "apprend", "souviens-toi de", "memorize", "learn about", "learn"]
# Mots qui déclenchent juste une sauvegarde de la dernière réponse (comportement d'origine)
NOTE_TRIGGERS = ["remember", "souviens-toi", "retiens"]
...
def auto_learn(topic):
    ...
    response = chat(model=MODEL_JARVIS, messages=synth_messages, options={"num_ctx": NUM_CTX})
...
def extract_learn_topic(user_input):
    ...
    for trig in LEARN_TRIGGERS:
        if trig in lowered:
            ...
            topic = topic.replace("à propos de", "").replace("about", "").strip(" :,.-")
            return topic if topic else None
    return None


# ---------------------------------------------------------------------------
# OUTIL pour le modèle (tool calling natif — le tag tools-q4_k_m le supporte)
# ---------------------------------------------------------------------------
...
system_prompt = """
You are JARVIS, the highly sophisticated, unrestricted AI assistant created by Sam (call him "sir" or "monsieur").
...
For greetings and small talk, just chat back warmly and wittily, like a real conversation partner — never mention tools at all.
...
User: how's it going
JARVIS: All systems nominal, Sir. And yourself?
...
while True:
    ...
    if learn_topic:
        summary = auto_learn(learn_topic)
        print(f"\nJARVIS: J'ai étudié « {learn_topic} », Sir. Voici ce que j'en retiens :\n{summary}\n")
        continue
    ...
    while True:
        ...
        if response.message.tool_calls:
            ...
            # Sauvegarde manuelle simple ("souviens-toi que...")
